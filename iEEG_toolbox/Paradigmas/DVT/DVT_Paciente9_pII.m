...
path_file=['D:\_INECO\Pacientes\Paciente9_AlfredoFarinelli\Masters\DVT\'];
   
path=[path_file '20141102_Farinelli_DVT_2_PII.edf' ]
channel=65;  
...
path_file=['D:\_INECO\Pacientes\Paciente9_AlfredoFarinelli\Logs\DVT\'];

% [trial_mark]=make_logmarkvector(path_file,name, sheet, range);

range='C182:C361' %Se perdió el primer trial
...
for i=1:max(size(trial_marks))
        ...
        mmark=find(abs(rest)<1)
        
        if ~isempty(mmark)
    
          
                EEG = pop_editeventvals(EEG,'changefield',{mmark(1) 'type' trial_marks(i)});
