...
trial_time=trial_time+micromed_time(4); %Response
...
if max(size(trial_time))==max(size(ev2mark))
    ...
    for i=1:max(size(ev2mark))
        ...
        if strcmp(EEG.event(1,ev2mark(i)).type,'chan2') 
         
            EEG = pop_editeventvals(EEG,'changefield',{ev2mark(i) 'type' mtype});
        
%         else
%               
%             EEG = pop_editeventvals(EEG,'changefield',{ev2mark(i)+1 'type' mtype});
%         
%         end
    end
...
for i=1:max(size(EEG.event))
    ...
    if strcmp (EEG.event(1,i).type,mtype) 
