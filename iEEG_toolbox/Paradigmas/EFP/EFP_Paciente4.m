...
path_file=['C:\Users\Fabri\Documents\INECO\Pacientes\Paciente4_Peirone\Masters\EFP\'];

% Patient 4
   
path=[path_file 'Paciente4.edf' ]
channel=56;
...
name='Rating_ResponseByPatient_Final.xls';
sheet='hoja1'


path_file=['C:\Users\Fabri\Documents\INECO\Pacientes\Paciente4_Peirone\Logs\EFP\'];
...
micromed_time=trial_time(2)+micromed_time+10;
...
for i=1:max(size(trial_marks))
    ...
    i
        ...
        mmark=find(abs(rest)<5)
        
        if ~isempty(mmark)
    
            if trial_marks(i)==1

                EEG = pop_editeventvals(EEG,'changefield',{mmark(1) 'type' 'int'});

            elseif  trial_marks(i)==2

                EEG = pop_editeventvals(EEG,'changefield',{mmark(1) 'type' 'acc'});

            elseif trial_marks(i)==3

                EEG = pop_editeventvals(EEG,'changefield',{mmark(1) 'type' 'neu'});
...
while p<size(EEG.event,2)
    ...
    if strcmp(EEG.event(p).type,'chan56') == 1
...
clear micromed_time
micromed_time=make_photodiodevector(EEG);
micromed_time=micromed_time-micromed_time(1,1)+10;
